...
#    For example: ./deploy-dist.sh solution_base_bucket my-solution v1.0.0 cn-northwest-1
...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ]; then
    echo "Please provide the base source bucket name(without region code), trademark approved solution name and version where the lambda code will eventually reside."
    echo "For example: ./deploy-dist.sh solution_base_bucket trademarked-solution-name v1.0.0 cn-northwest-1"
    exit 1
fi
